    ...
    ITaskAssigneeResolver assigneeResolver) : ITaskScheduleService
{
    ...
    public async Task DeleteAsync(Guid id, CancellationToken cancellationToken = default)
    {
        ...
        PagedResult<OperationalTask> pendingTasks = await unitOfWork.Repository<OperationalTask>().ListAsync(
            task => task.TaskScheduleId == schedule.Id && task.Status == OperationalTaskStatus.NotStarted,
            new PageRequest(1, PageRequest.MaximumPageSize),
            cancellationToken);
    ...
    public async Task SetActiveAsync(Guid id, bool isActive, CancellationToken cancellationToken = default)
    {
        ...
        if (isActive)
        {
            ...
            PagedResult<OperationalTask> cancelledTasks = await unitOfWork.Repository<OperationalTask>().ListAsync(
                task => task.TaskScheduleId == schedule.Id && task.Status == OperationalTaskStatus.Cancelled,
                new PageRequest(1, PageRequest.MaximumPageSize),
                cancellationToken);

            foreach (OperationalTask task in cancelledTasks.Items)
            {
