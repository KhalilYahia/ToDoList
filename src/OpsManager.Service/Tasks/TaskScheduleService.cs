    ...
    ITaskAssigneeResolver assigneeResolver) : ITaskScheduleService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        foreach (TaskDistribution dist in distributionsResult.Items)
        {
            ...
            if (distTasks.Items.Count > 0 && distTasks.Items.All(t => t.Status == OperationalTaskStatus.NotStarted))
            {
                ...
                List<OperationalTask> unassignedTasks = [];
                HashSet<Guid> matchedUserIds = [];

                foreach (OperationalTask task in distTasks.Items)
                {
                    if (task.AssigneeUserId.HasValue && resolvedAssignees.Any(a => a.UserId == task.AssigneeUserId.Value))
                    {
                        matchedUserIds.Add(task.AssigneeUserId.Value);
                    }
                    else
                    {
                ...
                List<ResolvedTaskAssignee> unhandledAssignees = resolvedAssignees
                    .Where(a => !matchedUserIds.Contains(a.UserId))
                    .ToList();

                // 1. Sync tasks for existing matched assignees
                foreach (OperationalTask task in distTasks.Items)
                {
                    if (task.AssigneeUserId.HasValue && matchedUserIds.Contains(task.AssigneeUserId.Value))
                    {
                        ...
                        unitOfWork.Repository<OperationalTask>().Update(task);
                        await SyncTaskItemsAsync(task.Id, templateItems.Items, schedule.OrganizationId, cancellationToken);
                    }
                }

                int commonCount = Math.Min(unassignedTasks.Count, unhandledAssignees.Count);

                // 2. Re-assign unassigned tasks to unhandled assignees in-place
                for (int i = 0; i < commonCount; i++)
                {
                    OperationalTask task = unassignedTasks[i];
                    if (task.Status == OperationalTaskStatus.Cancelled)
                    {
                        task.ResetToNotStarted(currentActorId, nowUtc, "Assignee updated on schedule.");
                    }

                    task.Assign(unhandledAssignees[i].UserId);
                    task.Reschedule(task.OccurrenceDate, newStart, newDue, false);
                    task.UpdateDetails(template.Title, template.Description, template.DefaultPriority, template.RequiresApproval);
                    unitOfWork.Repository<OperationalTask>().Update(task);
                    await SyncTaskItemsAsync(task.Id, templateItems.Items, schedule.OrganizationId, cancellationToken);
                ...
                // 3. Create new tasks for extra unhandled assignees
                for (int i = commonCount; i < unhandledAssignees.Count; i++)
                {
                    ResolvedTaskAssignee assignee = unhandledAssignees[i];
                ...
                // 4. Cancel extra unassigned tasks if assignees decreased
                for (int i = commonCount; i < unassignedTasks.Count; i++)
                {
                    OperationalTask extraTask = unassignedTasks[i];
                    if (extraTask.Status != OperationalTaskStatus.Cancelled)
                    {
    ...
    private async Task SyncTaskItemsAsync(
        Guid taskId,
        IReadOnlyList<TaskTemplateItem> templateItems,
        Guid organizationId,
        CancellationToken cancellationToken)
    {
        ...
        foreach (TaskTemplateItem templateItem in templateItems)
        {
    ...
    private async Task<List<OperationalTask>> GetTasksForScheduleAsync(
        Guid scheduleId,
        OperationalTaskStatus targetStatus,
        CancellationToken cancellationToken)
    {
        IReadOnlyList<Guid> distributionIds = await unitOfWork.Repository<TaskDistribution>().ProjectAsync(
            dist => dist.TaskScheduleId == scheduleId,
            dist => dist.Id,
            cancellationToken);

        List<OperationalTask> allTasks = [];
        int page = 1;
        while (true)
        {
            PagedResult<OperationalTask> pageResult = await unitOfWork.Repository<OperationalTask>().ListAsync(
                task => (task.TaskScheduleId == scheduleId || (task.TaskDistributionId.HasValue && distributionIds.Contains(task.TaskDistributionId.Value))) &&
                    task.Status == targetStatus,
                new PageRequest(page, PageRequest.MaximumPageSize),
                cancellationToken);

            allTasks.AddRange(pageResult.Items);
            if (allTasks.Count >= pageResult.TotalCount || pageResult.Items.Count == 0)
            {
                break;
            }
            page++;
    ...
    public async Task DeleteAsync(Guid id, CancellationToken cancellationToken = default)
    {
        ...
        TaskSchedule schedule = await GetEntityAsync(id, cancellationToken);
        schedule.Deactivate();
        unitOfWork.Repository<TaskSchedule>().Remove(schedule);

        DateTimeOffset nowUtc = clock.UtcNow;
        Guid actorId = currentUser.UserId ?? Guid.Empty;

        // Cancel all uncompleted NotStarted tasks generated by this schedule
        List<OperationalTask> pendingTasks = await GetTasksForScheduleAsync(
            schedule.Id,
            OperationalTaskStatus.NotStarted,
            cancellationToken);

        foreach (OperationalTask task in pendingTasks)
        {
            task.Cancel(actorId, nowUtc, "Task schedule was deleted.");
            unitOfWork.Repository<OperationalTask>().Update(task);
        }
    ...
    public async Task SetActiveAsync(Guid id, bool isActive, CancellationToken cancellationToken = default)
    {
        ...
        if (isActive)
        {
            ...
            List<OperationalTask> cancelledTasks = await GetTasksForScheduleAsync(
                schedule.Id,
                OperationalTaskStatus.Cancelled,
                cancellationToken);

            foreach (OperationalTask task in cancelledTasks)
            {
                task.ResetToNotStarted(actorId, nowUtc, "Task schedule was reactivated.");
                unitOfWork.Repository<OperationalTask>().Update(task);
            }
        }
        else
        {
            ...
            List<OperationalTask> unstartedTasks = await GetTasksForScheduleAsync(
                schedule.Id,
                OperationalTaskStatus.NotStarted,
                cancellationToken);

            foreach (OperationalTask task in unstartedTasks)
            {
                task.Cancel(actorId, nowUtc, "Task schedule was deactivated.");
                unitOfWork.Repository<OperationalTask>().Update(task);
            }
        }
